...
class Auth_Controller_Deployer extends BaseObject implements Auth_Controller_Interface
{
	...
	/**
	 *
	 * @var Auth_Developer_User|bool|null
	 */
	protected Auth_Developer_User|bool|null $current_user = null;
	...
	public function getCurrentUser(): Auth_Developer_User|bool
	{
		...
		if( $user_id ) {
			$this->current_user = Auth_Developer_User::get( $user_id );
		}
	...
	public function login( string $username, string $password ): bool
	{
		...
		$user = Auth_Developer_User::getByIdentity( $username, $password );
	...
	public function getCurrentUserHasPrivilege( string $privilege, mixed $value=null ): bool
	{
		...
		if(
			!$current_user ||
			!($current_user instanceof Auth_Developer_User)
		) {
			return false;
		}
	...
	{
		return $this->getCurrentUserHasPrivilege( Auth_Developer_Role::PRIVILEGE_VISIT_PAGE, $page->getKey() );
	}
